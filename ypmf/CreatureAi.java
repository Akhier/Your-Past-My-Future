...
public class CreatureAi {
	...
	protected boolean canRangedWeaponAttack(Creature other) {
		return creature.weapon() != null && creature.weapon().rangedAttackValue() > 0 && creature.canSee(other.x, other.y, other.z);
	}
	...
	protected boolean canUseBetterEquipment() {
		...
		for (Item item : creature.inventory().getItems()){
			...
			if (item.attackValue() + item.rangedAttackValue() > currentWeaponRating || isArmor && item.defenseValue() > currentArmorRating) {
				return true;
			}
		}
		return false;
